...
- [Ansible Automation: Dockerized Services \& NGINX on AWS Ubuntu](#ansible-automation-dockerized-services--nginx-on-aws-ubuntu)
...
[web]
your.ec2.ip.address ansible_user=ubuntu ansible_ssh_private_key_file=/path/to/pract.pem

```

- Replace `your.ec2.ip.address` with your EC2 instance’s public IP.
...
**File:** [images-install.yml](images-install.yml)
...
\# No port mapping since Ubuntu image does not expose a web server
...
**File:** [nginx-install.yml](nginx-install.yml)
...
- **Add or modify containers** in [images-install.yml](images-install.yml) under the `containers` variable.
- **Change the NGINX index page** content in [nginx-install.yml](nginx-install.yml) under the `copy` task.
